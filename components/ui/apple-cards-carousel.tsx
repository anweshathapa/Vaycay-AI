...
// 1. Using React.ReactNode instead of JSX.Element for React 19 compatibility
interface CarouselProps {
...
export const Card = ({
  ...
  const [open, setOpen] = useState(false);
  // 2. Fixed Ref error by using the non-null assertion (!)
  const containerRef = useRef<HTMLDivElement>(null!); 
  ...
  // 3. Cast the ref to satisfy the hook's specific type requirement if needed
...
export const BlurImage = ({
  ...
  // 4. Using Next.js <Image /> to properly handle 'fill' and avoid DOM attribute errors
  return (
    <Image
      className={cn(
